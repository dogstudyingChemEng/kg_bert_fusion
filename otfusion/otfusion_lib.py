...
# Encoder Fusion #
#----------------#
# [ otfusion_lib.py 的顶部... Imports, GroundMetric, dict_utils... ]
...
def encoder_fusion(args: dict, keys: dict, w_0: dict, w_1: dict, acts_0: dict, acts_1: dict, t_in: torch.Tensor, last_layer, device: torch.device, enc_key, log, alpha = 0.5, prev_out_acts = None):
    ...
    (CORRECTED to handle ViT (Pre-LN) and BERT (Post-LN) architectures)
    ...
    # --- (修正 1: 检查是否为 BERT Post-LN 模型) ---
    # 检查 'kg_bert' (来自 yaml) 或 'hf_bert_class' (来自原始文件)
    is_bert_model = args['model']['type'] == 'hf_bert_masked' or \
                    args['model']['type'] == 'hf_bert_class' or \
                    args['model']['type'] == 'kg_bert'

    # -----------------------------------------------
    # 路径 1: ViT (Pre-LN) 架构 (LN0 -> SA -> Resid1)
    # -----------------------------------------------
    if not is_bert_model:
        if (args['fusion']['fuse_norm']):
            log.info(' Fusing encoder {0}: norm 0 (Pre-SA)'.format(enc_key))
            ...
            w_norm_0_fused = copy.deepcopy(dict_get(ln0_keys, w_0)) # (修正: 之前这里错误地使用了 ln1_keys)
            t_out = t_in
    ...
    # -----------------------------------------------
    # 路径 2: BERT (Post-LN) 架构 (SA -> LN0 -> Resid1)
    # -----------------------------------------------
    else:
        t_out = t_in # BERT 在 SA 之前没有 LN0
        # (BERT 默认init ln0)
    ...
    if (args['fusion']['fuse_sa']):
        ...
        # (如果 SA 未融合, T-map 继承自 SA 输入, 即 t_out)

    # --- (修正 2: 根据架构重新排序 Resid1 和 LN) ---

    # -----------------------------------------------
    # 路径 1: ViT (Pre-LN) (Resid1 -> LN1)
    # -----------------------------------------------
    if not is_bert_model:
        # 1.a. ViT Resid1
        if t_in != None:
        ...
        t_resid = t_out # t_resid 保存 Resid1 的输出

        # 1.b. ViT LN1
        if args['fusion']['fuse_norm']:
            log.info(' Fusing encoder {0}: norm 1 (Pre-FFN)'.format(enc_key))
    ...
    # -----------------------------------------------
    # 路径 2: BERT (Post-LN) (LN0 -> Resid1)
    # -----------------------------------------------
    else:
        # 2.a. BERT LN0
        if args['fusion']['fuse_norm']:
            log.info(' Fusing encoder {0}: norm 0 (Post-SA)'.format(enc_key))
    ...
    if (args['fusion']['fuse_fc']):
        ...
        # (t_out 继承自 Resid1/LN1)

    # --- (修正 3: BERT LN1 融合) ---
    if is_bert_model:
        if (args['fusion']['fuse_norm']):
            log.info(' Fusing encoder {0}: norm 1 (Post-FFN)'.format(enc_key))
            w_norm_1_fused, t_out = ln_fusion(args = args, keys = keys, t_in = t_out, w_0 = dict_get(ln1_keys, w_0), w_1 = dict_get(ln1_keys, w_1), device = device)
        else:
            w_norm_1_fused = copy.deepcopy(dict_get(ln1_keys, w_1)) # (使用 model_1 的权重)
            # (t_out 继承自 FFN)

    # --- (公共部分: Resid2) ---
    if not last_layer:
        # Resid2: 结合 Resid1 的输出 (t_resid) 和 FFN/LN1 的输出 (t_out)
